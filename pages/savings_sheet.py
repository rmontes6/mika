...
layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            dcc.Graph(
                id='phasing-savings',
                figure=savings_functions.phasing_savings(df, 2026)
            )
        ], width=6),
        dbc.Col([
            dcc.Graph(
                id='phasing-final-year-estimation-income',
                figure=savings_functions.phasing_final_year_estimation_income(df, 2026)
            )
        ], width=6)
    ])
], fluid=True)
...
def expenses_KPI(value):
    return savings_functions.kpi_month_savings(df, value, 2026), \
           savings_functions.savings_estimation(df, value, 2026), \
           savings_functions.kpi_accumulated_savings(df, value, 2026)